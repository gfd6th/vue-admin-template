<template>
  <div class="overflow-hidden">
    ...
    >
      ...
      <li v-for="item in filterdItems" :key="item.id" class="w-1/2 text-center mb-4 cursor-pointer" :class="{selected: item.id===lightId}" @click="getLight(item.id)">
...
export default {
  ...
  methods: {
    ...mapActions('subsidebar', ['loadSubSidebar', 'loadLight']),
    ...
    async getLight(id) {
      await this.loadLight(id)
      this.lightId = id
      this.edit(this.$store.state.subsidebar.light)
    }
  }
}
</script>
