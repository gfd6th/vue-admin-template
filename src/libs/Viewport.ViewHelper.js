...
function ViewHelper(editorCamera, container) {
  ...
  this.render = function(renderer) {
    ...
    renderer.clearDepth()
    this.updatePosition(renderer)
    renderer.render(this, camera)
    // console.log(container, renderer)
  }

  this.updatePosition = function(renderer) {
    const dim = 128
    ...
    const left = rightSideBar.offsetLeft - width
    ...
    renderer && renderer.setViewport(left, bottom, dim, dim)
  }
  ...
  this.handleClick = function(event) {
    ...
    if (intersects.length > 0) {
      ...
      prepareAnimationData(object, new THREE.Vector3(0, 0, 0))
